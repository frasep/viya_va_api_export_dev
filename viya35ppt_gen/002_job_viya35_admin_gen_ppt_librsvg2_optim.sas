...
%macro print_images(filename, titl,folder);
	...
	data _NULL_;
	 	dcl odsout obj();
		obj.image(file:"&folder/&filename..png", height:"720", width:"1280");
	run;
%mend;
...
/***************************************************************************************************/
/***************************************************************************************************/
/* Get the list of job id and files from the save_root dir modified in the last 60 minutes. Indeed */
/* we don't want to take into account the obsolete jobs automatically deleted in the SAS Viya content */

Filename filelist pipe "find &SAVE_ROOTDIR -cmin -60 -type f -name '*.svg'";
...
data image_list;                                        
	 ...
	 image_filename=scan(filename,6,'/');
run;
...
data _null_;
	...
	/* Cleanse temporary directory before each report/user generation */
	SYSTASK COMMAND "rm -f &SAVE_ROOTDIR_TMP/*" WAIT;
	call execute('%generate_ppt_file('||user||','||report_id||','||job_id||','||name||'.pptx)');
run;
