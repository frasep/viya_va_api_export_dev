...
/* this job is executed in a specific compute context with allowxcmd privileges and visible only for the dmin user */
/*****************************************************************************************************************/
...
%macro transform_image(filename,basefilename);
    ...
    %put Dbg_Convert: filename timg pipe "rsvg-convert -a &filename > &save_rootdir_tmp/&basefilename..png";
...
%macro generate_ppt_file(user, report_id, job_id, pptxName);
	%put Dbg_Generate: &=user, &=report_id, &=job_id, &=pptxName;
	...
	data tb_dbg3;
	  ...
	  file ppt;
	  length line $ 2048;
	  line = cats('%let v_label="',sectionLabel,'";');
	  put line;
	  putlog line;
	  line = cats('%print_images(', cats("filename=", image_filename), ',titl=%superq(v_label),', cats("folder=","&SAVE_ROOTDIR_TMP"), ");");
	...
	/*SYSTASK COMMAND "rm -f &SAVE_ROOTDIR_TMP/*" WAIT;*/
...
proc sort data=WORK.image_list;
by image_filename;
quit; 
* ****************************************************************************************************;
...
proc sql;
	...
	/*where A.job_id=B.job_id and input(scan(A.image_filename,1,"_"),best.)=B.ordinal_images;*/
	where A.job_id=B.job_id and A.ordinal_images=B.ordinal_images;
quit;

proc sort data=loop_ppt_table;
	by report_id ordinal_images;
run;
...
data TB_DBG3;
	...
	message="Generating powerpoint for user : " ||user ||" and report " || report_id;
	appel='call execute(%generate_ppt_file('||strip(user)||','||strip(report_id)||','||strip(job_id)||','||strip(name)||'.pptx));';
run;

data _null_;
	set report_user_list;
	put "Generating powerpoint for user : " user " and report " report_id;
	call execute('%generate_ppt_file('||strip(user)||','||strip(report_id)||','||strip(job_id)||','||strip(name)||'.pptx);');
run;
